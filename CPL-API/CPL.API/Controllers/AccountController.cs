...
using CPL.API.App_Start;
using CPL.API.Models;
using CPL.API.Providers;
using CPL.API.Result;
...
namespace CPL.API.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public IdentityConfig.ApplicationUserManager UserManager
        {
            get
            {
                return _userManager ?? Request.GetOwinContext().GetUserManager<IdentityConfig.ApplicationUserManager>();
            }
            private set
            {
                _userManager = value;
        ...
        [HttpGet, Route("who-am-i")]
        public IHttpActionResult WhoAmI()
        {
            return this.Ok(new { name = this.User.Identity.Name });
        }
        ...
        [AllowAnonymous]
        [Route("Register")]
        public async Task<IHttpActionResult> Register(UserModel newUser)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            if (_userAuthService.IsUsernameTaken(newUser.UserName))
            {

                return _errorMessageService.BadRequestMessage(Request, "Username is in use.  Please select a new username a new username");

            }


            IdentityResult result = _authManager.RegisterUser(newUser);
        ...
        //    IdentityResult result = await _authManager.RegisterUser(userModel);
